...
class Address(metaclass=AddressMetaclass):
    ...
    def dict_contents(
        ...
        if self.addrType in (Address.localStationAddr, Address.remoteStationAddr):
            use_dict.__setitem__("addr", btox(self.addrAddr, ":"))
        if self.addrType in (
            Address.remoteStationAddr,
            Address.remoteBroadcastAddr,
            Address.globalBroadcastAddr,
        ):
            use_dict.__setitem__("net", self.addrNet)
            ...
            while True:
                ...
                if interface_port_match:
                    ...
                    if dns_addresses:
                        ...
                        if _debug:
                            IPv4Address._debug("    - ipv4_address_str: %r", ipv4_address_str)
...
class IPv6Address(Address, ipaddress.IPv6Interface):
    ...
    def __init__(
        ...
        # if the interface index is not set, try to find it
        if (interface_index == 0) and netifaces:
            for _interface in netifaces.interfaces():
                ipv6_addresses_dicts = netifaces.ifaddresses(_interface).get(
                    netifaces.AF_INET6, []
                )
                for adict in ipv6_addresses_dicts:
                    if adict["addr"].split("%")[0] == self.ip.compressed:
                        interface_index = socket.if_nametoindex(_interface)
                        if _debug:
                            IPv6Address._debug(
                                "    - interface_index (auto): %r", interface_index
                            )
                        break
                if interface_index:
                    break
...
class PDUData:
    ...
    def pdudata_contents(
        ...
        *,
        ...
        # add the data if it is not None and it is to be included
        v = self.pduData
        if include_data and v is not None:
            if isinstance(v, bytearray):
                use_dict.__setitem__("data", btox(v, ":"))
            elif hasattr(v, "dict_contents"):
                v = v.dict_contents(as_class=as_class, include_data=include_data)
